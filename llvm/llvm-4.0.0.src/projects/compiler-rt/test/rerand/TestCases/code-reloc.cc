...
// RUN: %clangxx_rerand_code -mllvm -rerand-reloc -O0 %s -o %t2
// RUN: %clangxx_rerand -mllvm -rerand-reloc -O0 %s -o %t3
...
void shuffle() {
  ...
  int res = mprotect((void*)aligned, size, PROT_READ|PROT_WRITE|PROT_EXEC);
  if (res == -1) {
  ...
  // ah .. funny bug!
  // moved code contains parts that must be relocated (recursively relocated ..)
  // to solve this, do relocation first before copy!
  // but the current implementation does not support it
  // (we don't know the moved location before actually moving)
  // FIXME
  relocate_code();
#endif
}
...
int main(void) {
  ...
  // NCHECK: [RT] (__rerand_code_return
  // NCHECK: {{end of main}}
  // PCHECK: {{end of main}}
  printf("[*] end of main\n");
  return 0;
