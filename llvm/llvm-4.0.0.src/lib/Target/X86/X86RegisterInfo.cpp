...
BitVector X86RegisterInfo::getReservedRegs(const MachineFunction &MF) const {
  ...
  // OBFUSCURO: Reserve R14, R15 for temporal variables.
  ...
  // Byunggill : Reserve one more register, R13
  Reserved.set(X86::R13);
  Reserved.set(X86::R13D);
  Reserved.set(X86::R13W);
  Reserved.set(X86::R13B);
  // ORAM: Reserve XMM0-15 for oram stash
/*
  for (unsigned i = 0; i < 16; i++) {
    for (MCRegAliasIterator AI(X86::XMM0 + i, this, true); AI.isValid(); ++AI)
      Reserved.set(*AI);
    for (MCRegAliasIterator AI(X86::YMM0 + i, this, true); AI.isValid(); ++AI)
      Reserved.set(*AI);
  }
*/
