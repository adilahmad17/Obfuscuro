...
/*
  class Instrumenter{
      public:
        InstCounter(TargetMachine &TM, X86AsmPrinter printer) : TM(TM), printer(this){}
        void reset(MachineFunction &F);
        bool decideInstrumentType(MCInst &Inst, bool& isLoadStore, bool& isRefByRegisterOffset,  unsigned& properMOVrr, unsigned& movOperationSize);
        bool instrumentMOVmr(MCInst &Instr, bool isRefByRegisterOffset, unsigned properMOVrr, unsigned movOperationSize);
        bool instrumentMOVrm(MCInst &Instr, bool isRefByRegisterOffset, unsigned properMOVrr);
        bool instrumentMOV(MCInst &Instr);
  ...
  */
  //byunggill: InstCounter from SGXShield
  class InstCounter {
      public:
  ...
  void DelayWithNop(unsigned int numberOfNop);
  void EmitSplitInstructions();
...
public:
  //BG
  void X86AsmPrinterDoBeforeEmitingLabel() override;
  ...
  unsigned int maximumCallFrameSize = 0;

  bool isOblivious = false;
  bool isRegularCall = false;
  bool isRegularReturn = false;
  
  //for the emission of first basic block
  bool obliviousFirstLabelEmitted = false;
