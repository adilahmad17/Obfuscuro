...
void Verifier::verifyDominatesUse(Instruction &I, unsigned i) {
  ...
  if (!DT.dominates(Op, U)) {
    errs() << "Instruction does not dominate all uses!\n";
    errs() << "funcname: " << I.getParent()->getParent()->getName() << "\n";
    errs() << "OP: " << *Op << "\n";

    const DebugLoc &location = Op->getDebugLoc();
    if (location) {
      errs() << "location : " << location.getLine() << ":" << location.getCol() << "\n";
    }
