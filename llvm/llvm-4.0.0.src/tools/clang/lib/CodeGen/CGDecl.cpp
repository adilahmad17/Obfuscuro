...
namespace {
  ...
  struct RerandStackFree final : EHScopeStack::Cleanup {
    Address addr;
    RerandStackFree(Address addr): addr(addr) {}

    void Emit(CodeGenFunction &CGF, Flags flags) override {
      llvm::Type *RetTy = CGF.VoidTy;
      llvm::Type *PtrTy = addr.getType();
      /*
      llvm::Value *F = CGF.CGM.getModule().getOrInsertFunction(
        "__rerand_stack_free", RetTy, PtrTy, nullptr);
      assert(F && "Rerand failed to locate the function __rerand_stack_free");
      CGF.Builder.CreateCall(F, addr.getPointer());
      */
    }
  };
...
CodeGenFunction::EmitAutoVarAlloca(const VarDecl &D) {
  bool shouldReplaceAlloca = false;

/*
  if (SanOpts.has(SanitizerKind::RerandStack)) {
    // TODO: blacklisting if required.
    shouldReplaceAlloca = true;
  }
*/
  ...
  if (Ty->isConstantSizeType()) {
    ...
    if (NRVO) {
      ...
      if (shouldReplaceAlloca) {
        llvm::Value *Size = llvm::ConstantInt::get(
          CGM.SizeTy, CGM.getDataLayout().getTypeAllocSize(allocaTy));
        llvm::Type *RetTy = llvm::PointerType::getUnqual(allocaTy);
        llvm::Type *SizeTy = Size->getType();
        llvm::Value *F = CGM.getModule().getOrInsertFunction(
          "__rerand_stack_alloc", RetTy, SizeTy, nullptr);
        assert(F && "Rerand failed to locate the function __rerand_stack_alloc");

        llvm::Value *AllocCall = Builder.CreateCall(F, Size);

        address = Address(AllocCall, allocaAlignment);
        emission.IsReplacedAlloca = true;
      } else {
      ...
      if (HaveInsertPoint() && !shouldReplaceAlloca && !IsMSCatchParam) {
...
void CodeGenFunction::EmitAutoVarCleanups(const AutoVarEmission &emission) {
  ...
  if (emission.useLifetimeMarkers() && !emission.IsReplacedAlloca)
  ...
  if (emission.IsReplacedAlloca)
    EHStack.pushCleanup<RerandStackFree>(NormalCleanup,
                                         emission.getAllocatedAddress());
